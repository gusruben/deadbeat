...
func update_animation():
	var look_direction = "right" if GameManager.player.position.x > position.x else "left"
	
	if velocity.x == 0:
		$AnimatedSprite2D.play("idle_look_" + look_direction)
	elif velocity.x > 0:
		$AnimatedSprite2D.play("run_right_look_" + look_direction)
	else:
		$AnimatedSprite2D.play("run_left_look_" + look_direction)
